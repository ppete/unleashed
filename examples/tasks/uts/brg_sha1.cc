...
void rng_init(RNG_state *newstate, int seed)
{
  ...
  for (i=0; i < 16; i++)
    gen.state[i] = 0;
  gen.state[16] = (uint8_t) (0xFF & (seed >> 24));
  gen.state[17] = (uint8_t) (0xFF & (seed >> 16));
  gen.state[18] = (uint8_t) (0xFF & (seed >> 8));
  gen.state[19] = (uint8_t) (0xFF & (seed >> 0));
...
void rng_spawn(RNG_state *mystate, RNG_state *newstate, int spawnnumber)
{
	...
	bytes[0] = (uint8_t) (0xFF & (spawnnumber >> 24));
	bytes[1] = (uint8_t) (0xFF & (spawnnumber >> 16));
	bytes[2] = (uint8_t) (0xFF & (spawnnumber >> 8));
	bytes[3] = (uint8_t) (0xFF & spawnnumber);
