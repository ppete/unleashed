...
#ifndef _UAB_QUEUE_HPP
#define _UAB_QUEUE_HPP 1
  ...
  struct queue
  {
      ...
      std::pair<_Tp, bool> deq()
      {
        ...
        do
        {
          ...
          if (dummy == tail.load(std::memory_order_acquire)) return std::make_pair(_Tp(), false);
...
namespace locking
{
  ...
  struct queue
  {
      ...
      {
        queue_elem<value_type>* const entry = new queue_elem<value_type>(e);
