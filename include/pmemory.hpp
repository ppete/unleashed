...
///            operation that uses epoc, no memory can be freed.
...
namespace lockfree
{
  ...
  {
    static const size_t RECOLLECTION_PERIOD = 1500;
    ...
    epoch_data()
    : next(nullptr), epoch(1), collepoch(RECOLLECTION_PERIOD), rmvd(1)
    {
      assert(!rmvd.empty());
    }
  };
  ...
  {
      static const size_t RECOLLECTION_PERIOD = 1000; ///< collect every 8 operations
      ...
      void endop()
      {
        ...
        // using relaxed instead of seq_cst is conservative, but
        //   may delay freeing of some objects
